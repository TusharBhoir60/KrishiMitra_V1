...
const mlClient = require('./nodejs_client');
...
router.post('/price', async (req, res) => {
  try {
    ...
    // Graceful fallback when ML service is down
...
router.post('/demand', async (req, res) => {
  try {
    const {
      cropName, state, month, season,
      historicalDemandScores, historicalPrices, forecastWeeks,
    } = req.body;
...
router.post('/quality', upload.single('image'), async (req, res) => {
  try {
    ...
    const result = await mlClient.analyzeCropQuality(
      req.file.buffer,
      req.file.mimetype,
      cropName || '',
    );
...
router.post('/listing-insights', async (req, res) => {
  try {
    ...
    const params = { cropName, state, district, quantity: Number(quantity), month: Number(month), season };

    // Run price + demand in parallel for speed
