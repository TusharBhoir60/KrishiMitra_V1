...
export const ActiveJob = () => {
  ...
  useEffect(() => {
    ...
    fetchJobSafe();
    const interval = setInterval(fetchJobSafe, 15000); // Poll fast for status changes
  ...
  const handleAction = async (actionStr) => {
    try {
      ...
      } else if (actionStr === 'at_buyer') {
        await deliveryApi.updateJobStatus(id, { status: 'at_buyer' });
        toast.success('Status updated. Collect buyer OTP to complete delivery.');
  ...
  const currentStep =
    job.status === 'scheduled' ? 1
    : job.status === 'at_farm' ? 2
    : job.status === 'in_transit' ? 3
    : job.status === 'at_buyer' || job.status === 'delivered' || job.status === 'completed' ? 4
    : 0;
