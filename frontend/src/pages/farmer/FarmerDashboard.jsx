...
export const FarmerDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const earnings  = completed.reduce((sum, o) => sum + (o.orderDetails?.cropAmount || 0), 0);
  ...
  const statCards = [
    {
      label: t('farmer.dashboard.totalListings'), value: data.listings.length,
      icon: ListChecks, color: 'green',
      to: '/farmer/listings', sub: 'active crops',
    },
    {
      label: t('farmer.dashboard.activeOrders'), value: data.activeOrders.length,
      icon: ShoppingCart, color: 'blue',
      to: '/farmer/orders?tab=active', sub: 'in progress',
    },
    {
      label: t('farmer.dashboard.pendingDeliveries'), value: data.pendingDeliveries,
      icon: Truck, color: 'amber',
      to: '/farmer/orders?tab=active', sub: 'en route',
    },
    {
      label: t('farmer.dashboard.totalEarnings'), value: formatINR(data.totalEarnings),
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 md:pb-8">
      ...
      <div className="bg-gradient-to-r from-farm-green via-farm-mid to-farm-light px-6 py-8 md:py-10 md:px-10">
        <p className="text-farm-pale/70 text-xs font-semibold uppercase tracking-widest mb-1">{t('farmer.dashboard.portal')}</p>
        <h1 className="text-2xl md:text-3xl font-display font-bold text-white mb-1">
          {t('farmer.dashboard.welcome')}, {(user?.name || user?.fullName)?.split(' ')[0]} 🌾
        </h1>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-bold text-farm-dark flex items-center gap-2 text-sm">
                <ShoppingCart size={17} className="text-farm-green" /> {t('farmer.dashboard.activeOrders')}
              </h2>
              <Link to="/farmer/orders" className="text-xs text-farm-green hover:underline font-medium flex items-center gap-1">
                {t('farmer.dashboard.viewAll')} <ArrowRight size={13} />
              </Link>
            </div>
            <div className="divide-y divide-gray-50">
              {data.activeOrders.length === 0 ? (
                <div className="py-14 text-center text-gray-400">
                  ...
                  <p className="text-sm font-medium">{t('farmer.dashboard.noActiveOrders')}</p>
                  <p className="text-xs mt-1">{t('farmer.dashboard.newOrdersAppear')}</p>
                </div>
              ) : (
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
              <h2 className="font-bold text-farm-dark text-sm mb-3">{t('farmer.dashboard.quickActions')}</h2>
              <div className="space-y-2">
                ...
                >
                  <Plus size={17} /> {t('farmer.dashboard.addListing')}
                </Link>
                <Link
                  to="/farmer/orders?tab=pending"
                ...
                >
                  ...
                  {t('farmer.dashboard.pendingOrders')}
                  {data.pendingOrders.length > 0 && (
