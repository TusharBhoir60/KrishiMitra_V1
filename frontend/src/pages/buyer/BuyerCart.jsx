...
export const BuyerCart = () => {
  ...
  const { t } = useLanguage();
  const { cartItems, cartTotal, updateCartQuantity, removeFromCart } = useBuyerCart();
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="font-display text-3xl font-bold text-farm-dark">{t('buyer.cart.subtitle')}</h1>
        </div>
        <p className="text-sm text-gray-500">{cartItems.length} item{cartItems.length === 1 ? '' : 's'} {t('buyer.cart.selectedItems')}</p>
      </div>
      ...
      {cartItems.length === 0 ? (
        <div className="rounded-3xl border-2 border-dashed border-green-100 bg-white p-12 text-center shadow-sm">
          ...
          <h2 className="font-display text-2xl font-bold text-farm-dark">{t('buyer.cart.emptyTitle')}</h2>
          <p className="mt-2 text-sm text-gray-500">{t('buyer.cart.emptySub')}</p>
          <button
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
            ...
            {cartItems.map((item) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-[#8a5a2b]">{item.farmer?.name || t('buyer.marketplace.verifiedFarmer')}</p>
                  <p className="mt-1 text-sm text-gray-500">{item.farmer?.location?.district || t('buyer.cart.localFarm')} · {formatINR(item.pricePerKg)}/kg</p>
          ...
          <div className="h-fit rounded-3xl border border-[#e8decb] bg-[#fcfaf5] p-6 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8c7a5a]">{t('buyer.cart.orderSummary')}</p>
            <h2 className="mt-1 font-display text-2xl font-bold text-farm-dark">{t('common.checkout')}</h2>
            <div className="mt-6 space-y-3 text-sm text-gray-600">
              <div className="flex items-center justify-between">
                <span>{t('buyer.cart.subtotal')}</span>
                <span className="font-semibold text-farm-dark">{formatINR(cartTotal)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>{t('buyer.cart.service')}</span>
                <span className="font-semibold text-farm-dark">{t('buyer.cart.free')}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>{t('buyer.cart.delivery')}</span>
                <span className="font-semibold text-farm-dark">{t('buyer.cart.calculatedByFarmer')}</span>
              </div>
            </div>
            <div className="my-5 h-px bg-[#e8decb]" />
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-600">{t('buyer.cart.total')}</span>
              <span className="text-2xl font-bold text-farm-green">{formatINR(cartTotal)}</span>
            </div>
            <button
              onClick={handleCheckout}
              className="mt-6 w-full rounded-2xl bg-farm-green px-5 py-3.5 text-sm font-bold text-white transition-all hover:-translate-y-0.5 hover:bg-farm-dark flex items-center justify-center gap-2"
            >
