      ...
      <div className="relative h-52 overflow-hidden bg-gradient-to-br from-green-50 to-[#eee2ca]">
        ...
        <div className="absolute left-3 top-3 flex flex-col gap-1">
          ...
          <span className="inline-flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-[#8a5a2b]">
            <Leaf size={10} /> {t('buyer.marketplace.freshHarvest')}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="mb-2 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-[#8a5a2b]">{crop.farmer?.name || t('buyer.marketplace.verifiedFarmer')}</p>
          </div>
        ...
        <p className="mb-4 flex items-center gap-1.5 text-sm text-gray-500">
          <MapPin className="h-4 w-4" /> {crop.farmer?.location?.district || t('buyer.marketplace.villageMarket')}
        </p>

        <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
          <div className="rounded-2xl bg-[#f7fbf4] p-3">
            <p className="text-xs uppercase tracking-wide text-gray-400">{t('buyer.marketplace.available')}</p>
            <p className="font-semibold text-farm-dark">{crop.availableQty} kg</p>
          </div>
          <div className="rounded-2xl bg-[#fcfaf5] p-3">
            <p className="text-xs uppercase tracking-wide text-gray-400">{t('buyer.marketplace.minimum')}</p>
        ...
        <div className="mt-4 flex items-center gap-1 text-sm text-amber-600">
          ...
          <span className="text-gray-400">{t('buyer.marketplace.trustedByBuyers')}</span>
        </div>
        ...
        <div className="mt-auto pt-5">
          ...
          >
            <ShoppingCart size={16} /> {t('buyer.marketplace.addToCart')}
          </button>
        </div>
      </div>
    </div>
...
export const Marketplace = () => {
  ...
  const filters = [
    { key: 'all', label: t('buyer.marketplace.all') },
    { key: 'vegetables', label: t('buyer.marketplace.vegetables') },
    { key: 'fruits', label: t('buyer.marketplace.fruits') },
    { key: 'grains', label: t('buyer.marketplace.grains') },
    { key: 'pulses', label: t('buyer.marketplace.pulses') },
    { key: 'spices', label: t('buyer.marketplace.spices') },
    { key: 'platform-assured', label: t('buyer.marketplace.platformAssured') },
  ];
    ...
    const fetchCrops = async () => {
      try {
        ...
        if (activeFilter !== 'all') {
          ...
          else params.category = activeFilter.endsWith('s') ? activeFilter.slice(0, -1) : activeFilter;
        }
    ...
    <div className="pb-10 pt-6">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 rounded-[2rem] bg-[linear-gradient(135deg,#1b4332_0%,#52b788_100%)] px-6 py-8 text-white shadow-lg shadow-green-100 sm:px-8">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-green-100">{t('buyer.marketplace.badge')}</p>
          <h1 className="mt-2 font-display text-4xl font-bold">{t('buyer.marketplace.heading')}</h1>
          <p className="mt-2 max-w-2xl text-sm text-green-50/90 sm:text-base">
            {t('buyer.marketplace.subheading')}
          </p>
        </div>
        ...
        <div className="mb-8 space-y-4">
          <div className="sticky top-[84px] z-20 flex gap-2 md:static md:top-auto">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} type="text" placeholder={t('buyer.marketplace.searchPlaceholder')} className="w-full rounded-2xl border border-green-100 bg-white py-3 pl-12 pr-4 outline-none ring-0 transition-shadow focus:shadow-md" />
            </div>
            <button className="flex items-center gap-2 rounded-2xl border border-green-100 bg-white px-4 py-3 font-medium text-gray-600 shadow-sm transition-colors hover:bg-green-50">
              <SlidersHorizontal className="h-5 w-5" /> <span className="hidden sm:inline">{t('buyer.marketplace.filters')}</span>
            </button>
          </div>

          <div className="hide-scrollbar flex gap-2 overflow-x-auto pb-2">
            {filters.map((f) => (
              <button key={f.key} onClick={() => setSearchParams({ category: f.key })} className={`shrink-0 rounded-full px-4 py-2 text-sm font-semibold transition-colors ${activeFilter === f.key ? 'bg-farm-green text-white shadow-md shadow-green-100' : 'border border-green-100 bg-white text-gray-600 hover:bg-green-50'}`}>
                {f.label}
              </button>
            ))}
          </div>
        ...
        <p className="mb-6 text-sm font-medium text-gray-500">{crops.length} {t('buyer.marketplace.productsAvailable')}</p>
        ...
        {loading ? (
          ...
          <div className="mt-6 rounded-3xl border-2 border-dashed border-green-100 bg-white p-12 text-center">
            ...
            <h3 className="font-display font-bold text-xl text-farm-dark mb-2">{t('buyer.marketplace.noCrops')}</h3>
            <p className="text-gray-500">{t('buyer.marketplace.noCropsSub')}</p>
          </div>
        ) : (
