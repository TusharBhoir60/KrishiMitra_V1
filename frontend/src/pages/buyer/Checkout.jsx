...
// FIX: removed useOrders import — cart checkout now goes directly to the backend API
...
export const Checkout = () => {
  ...
  const [deliveryMethod, setDeliveryMethod] = useState('farmer_delivers');
  ...
  const transportBreakdown = useMemo(() => {
    if (deliveryMethod !== 'platform_transporter') return [];
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      } else if (failed.length < cartItems.length) {
        clearCart();
        toast.error(`Some orders failed: ${failed.join(', ')}`);
        navigate('/buyer/orders');
      } else {
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white rounded-3xl border border-green-100 p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-8 h-8 bg-farm-green rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0">A</div>
                ...
                return (
                  <label
                    ...
                    className={`flex items-start gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all ${
                      isSelected ? 'border-farm-green bg-green-50/60' : 'border-gray-100 hover:border-green-200 bg-white'
                    }`}
                  >
                    <input
                      type="radio"
                    ...
                    />
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${isSelected ? 'bg-farm-green text-white' : 'bg-gray-100 text-gray-400'}`}>
                      <Icon size={18} />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <p className="font-semibold text-farm-dark">{m.label}</p>
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${m.id === 'buyer_pickup' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                          {m.cost}
                        </span>
                      </div>
                ...
                {transportBreakdown.map((tb, i) => (
                  <div key={i} className="text-sm">
                    ...
                    <div className="flex flex-wrap gap-4 mt-1 text-blue-700">
                      ...
                      {tb.days && <span className="flex items-center gap-1"><Truck size={13} />{tb.days} days</span>}
                      <span className="font-bold">Transport: {formatINR(tb.cost)}</span>
                    </div>
                  </div>
                ))}
                {transportBreakdown.length === 0 && (
                  <p className="text-sm text-blue-700">Update your profile district to see transport estimate.</p>
                )}
              </div>
            )}
          </section>
          ...
          <section className="bg-white rounded-3xl border border-green-100 p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-8 h-8 bg-farm-green rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0">B</div>
          ...
          <section className="bg-white rounded-3xl border border-green-100 p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-8 h-8 bg-farm-green rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0">C</div>
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${
                      isSelected ? 'border-farm-green bg-green-50 text-farm-green' : 'border-gray-100 text-gray-400 hover:border-green-200'
                    }`}
                  >
                    <Icon size={22} />
            ...
            {paymentMethod === 'upi' && (
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-farm-green focus:border-transparent"
                />
                <p className="text-xs text-gray-400">* Payment processing is not yet integrated — orders are placed as pending.</p>
              </div>
            )}
        ...
        <div className="space-y-4 lg:sticky lg:top-4 h-fit">
          <div className="bg-white rounded-3xl border border-[#e8decb] p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8c7a5a] mb-3">{t('buyer.checkout.orderItems')}</p>
            <div className="space-y-3">
              {cartItems.map((item) => (
                <div key={item._id} className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl bg-green-50 overflow-hidden shrink-0">
                    {item.images?.[0]?.url ? (
                      <img src={item.images[0].url} alt={item.cropName} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">{item.quantity} kg × {formatINR(item.pricePerKg)}</p>
                  </div>
                  <p className="text-sm font-bold text-farm-green shrink-0">{formatINR(item.quantity * item.pricePerKg)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#fcfaf5] rounded-3xl border border-[#e8decb] p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8c7a5a] mb-4">{t('buyer.checkout.priceSummary')}</p>
            <div className="space-y-3 text-sm text-gray-600">
              ...
              <div className="flex justify-between">
                ...
                <span className={`font-semibold ${totalTransport === 0 ? 'text-green-600' : 'text-farm-dark'}`}>
