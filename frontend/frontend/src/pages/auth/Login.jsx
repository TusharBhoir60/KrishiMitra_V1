...
export const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      toast.success(`Logged in as ${res.data.user.role}! (Demo Mode)`);
  ...
  return (
    <div className="min-h-screen bg-farm-cream flex items-center justify-center p-6 pb-24">
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 max-w-md w-full">
        ...
        <h2 className="font-display text-3xl text-farm-dark mb-6 text-center">Welcome back</h2>
        
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            <label className="block font-display text-gray-700 mb-2">Email</label>
            <input 
              type="email" 
              required
              value={email}
        ...
        <div className="mt-6 text-center font-body text-gray-600">
          Don't have an account? <Link to="/register" className="text-farm-green font-semibold hover:underline">Create account →</Link>
        </div>
      </div>
    </div>
  );
